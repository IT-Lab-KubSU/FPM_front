...
export const NewCard = ({card}: { card: INewDTO }) => {
    return <div
        className={`relative flex flex-col justify-between bg-white dark:bg-zinc-100  dark:text-zinc-800 w-[300px] h-full p-8 rounded-xl drop-shadow-md duration-700`}>
        <div className="newCard mb-8">
            <div className={'w-full h-[150px] flex justify-center items-center'}>
                <Image src={card.image} alt={card.title} height={150} width={300}
                       className={`rounded-xl drop-shadow-md max-h-[150px]`}/>
            </div>
            <br/>
            <p className="line-clamp-4">{card.title}</p>
        </div>
